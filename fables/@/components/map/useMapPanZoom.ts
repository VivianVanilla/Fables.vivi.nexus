...
export function useMapPanZoom(initial: { x: number; y: number } = { x: 40, y: 40 }) {
  ...
  // Zooms toward the cursor rather than the content's transform-origin
  // (top-left) — keep whatever world point is currently under the mouse
  // pinned to the same screen position, panning to compensate for the
  // zoom change instead of leaving pan untouched.
  function onWheel(e: React.WheelEvent) {
    // e.preventDefault()
    const newZoom = Math.min(2.5, Math.max(0.3, +(zoomRef.current - e.deltaY * 0.001).toFixed(2)))
    const rect = surfaceRef.current?.getBoundingClientRect()
    const left = rect?.left ?? 0
    const top = rect?.top ?? 0
    const worldX = (e.clientX - left - panRef.current.x) / zoomRef.current
    const worldY = (e.clientY - top - panRef.current.y) / zoomRef.current
    setZoom(newZoom)
    setPan({ x: (e.clientX - left) - worldX * newZoom, y: (e.clientY - top) - worldY * newZoom })
  }
