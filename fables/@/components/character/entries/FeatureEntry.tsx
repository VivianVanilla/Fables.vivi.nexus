...
// Container name prefix — inlined from public/backpack-svgrepo-com.svg
// (stroke swapped for currentColor so it picks up the surrounding text
// color instead of the source file's fixed navy) rather than a 🎒 emoji
// character, which renders inconsistently across platforms/OS emoji fonts.
function BackpackIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="inline-block size-3.5 -mt-0.5 align-middle mr-0.5" aria-hidden>
    ...
    <div className={`rounded-xl border-2 overflow-hidden shrink-0 ${magicStyle ? "" : "border-white/10"} ${isAnimatedStyle(magicStyle) ? "" : theme.box}`}
      ...
      <div {...dragAttrs}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {/* pointer-coarse:ml-2 — nudged right on mobile. */}
          <span className="text-[10px] text-white/30 shrink-0 w-3 pointer-coarse:ml-2">{expanded ? "▼" : "▶"}</span>

          <span className="min-w-24 flex-1 text-sm font-semibold text-white truncate">
            {feature.isContainer && <BackpackIcon />}
            {magicStar && "✨ "}
