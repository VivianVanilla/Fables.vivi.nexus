...
// The two built-in CSS presets (Void Corruption, Galaxy) were removed —
// their many-layer gradients plus the SVG turbulence grain, painted by
// every card with background-attachment:fixed, were genuinely laggy on
// mobile. Only "custom" (an uploaded photo — CharacterData.bgImageCustomUrl)
// remains as a Card Overlay option. The interface/keys are kept so the
// custom path and any old saved `bgImageStyle` values still resolve
// (an unknown key just yields no overlay, same as "none").
