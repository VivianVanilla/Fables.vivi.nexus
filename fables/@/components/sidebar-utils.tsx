...
// Personal sidebar tree — every item here is owned by the current user (party
// sharing now lives entirely in the Party Server instead of on individual
// note objects), so this is a plain owner-only tree with no viewer-relative
// position overrides.
...
export function moveToRoot(
  ...
  return prevItems.map((item) =>
    item.id === draggedId ? { ...item, position: newPosition, parent_id: null } : item
  )
}
...
export function applyDrop(
  ...
  if (isNoNesting(dragged) && drop.position === "inside") return prevItems
  ...
  if (drop.position === "inside") {
    ...
    return prevItems.map((item) =>
      item.id === draggedId ? { ...item, position: siblings.length, parent_id: target.id } : item
    )
  }
  ...
  return prevItems.map((item) => {
    if (item.id === draggedId) {
      return { ...item, position: positionMap.get(item.id) ?? 0, parent_id: newParentId }
    }
